...
""",
    ...
    def test_opencode_install_check_remove(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.assertEqual(setup_cli.main(["add", "opencode", "--cwd", str(root), "--project", "demo", "--no-db"]), 0)
    ...
    def test_cursor_install_check_remove(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.assertEqual(setup_cli.main(["cursor", "--cwd", str(root), "--project", "demo", "--no-db"]), 0)
    ...
    def test_hermes_install_check_remove(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.assertEqual(setup_cli.main(["hermes", "--cwd", str(root), "--project", "demo", "--no-db"]), 0)
            doc = root / "HERMES.md"
